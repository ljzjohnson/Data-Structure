 ...
 * 
 * @author luzhongguo
 * @version 1.0, 2016年5月12日
 */
public class BranchAndBound {
	float[][] a;// 图G的邻接矩阵
	public BranchAndBound(float[][] a) {
		this.a = a;
	}
				...
				for (int i = enode.s + 1; i < n; i++) {
					if (a[x[enode.s]][x[i]] != -1) {
						...
						if (b < bestc) {
							...
							xx[enode.s + 1] = x[i];
							xx[i] = x[enode.s + 1];
							HeapNode node = new HeapNode(b, cc, rcost,
									enode.s + 1, xx);
							heap.add(node);
							Collections.sort(heap);
						}
	...
	public static void main(String[] args) {
		// int n=4;
		// float[][]
		// a={{0,0,0,0,0},{0,-1,30,6,4},{0,30,-1,5,10},{0,6,5,-1,20},{0,4,10,20,-1}};//a下标从1开始，0用来凑数；-1表示不同，1表示连通
		int n = 5;
		float[][] a = {{0, 0, 0, 0, 0, 0}, {0, -1, 5, -1, 7, 9},
				{0, 5, -1, 10, 3, 6}, {0, -1, 10, -1, 8, -1},
				{0, 7, 3, 8, -1, 4}, {0, 9, 6, -1, 4, -1}};// a下标从1开始，0用来凑数；-1表示不同，1表示连通
		BranchAndBound b = new BranchAndBound(a);
		int[] v = new int[n + 1];
